...
# A failed devicetree load must reboot rather than fall through to `linux`: the
# kernel would boot the vendor DTB, ufshcd-qcom would never probe, and rootwait
# would hang forever with the trial boot already counted.
printf '%s\n' "$BODY" | awk '/if ! devicetree/,/^[[:space:]]*fi/' | grep -q 'reboot'
check $? "a failed 'devicetree' load reboots to fall back"
