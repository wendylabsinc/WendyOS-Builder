...
def _bitbake_list(text: str, name: str) -> list[str]:
    # Only a plain assignment is readable from here; an override or append would be applied
    # by BitBake and silently missed, so refuse rather than check a stale list.
    if re.search(rf"^{name}(:\w+|\s*[?+:]=)", text, re.MULTILINE):
        raise ValueError(f"{name} uses an assignment this check cannot read")
...
def load_contract(path: Path = DEFAULT_CONTRACT) -> Contract:
    ...
    for entry in _bitbake_list(text, "WENDYOS_GAME_CONTROLLER_MODULES"):
        ...
        if not package:
            raise ValueError(f"{symbol} names no module package in the contract")
        modules[symbol] = package
    builtin = frozenset(_bitbake_list(text, "WENDYOS_GAME_CONTROLLER_BUILTIN"))
    if not builtin or not modules:
        raise ValueError(f"{path} has an empty list; those symbols would go unchecked")
...
def manifest_has(packages: set[str], expected: str) -> bool:
    ...
    The suffix must look like a version: kernel-module-bluetooth-6lowpan is a different
    driver, not a versioned kernel-module-bluetooth.
    """

    versioned = re.compile(rf"{re.escape(expected)}-\d+\.\d+")
    return expected in packages or any(versioned.match(package) for package in packages)
