...
// The helper has to clear a generic image path it inherits, rather than rely on
// every caller to avoid setting one.
func TestApplyEDLFlashBundleClearsAnInheritedImagePath(t *testing.T) {
	meta := VersionMetadata{Path: "stale.img", SizeBytes: 999, Checksum: "stale"}
	applyEDLFlashBundle(&meta, "dragonwing-iq-9075", "9075.qcomflash.tar.gz", 42, "sha9075")
	if meta.Path != "" || meta.SizeBytes != 0 || meta.Checksum != "" {
		t.Errorf("9075 kept a generic image route: path=%q size=%d sum=%q", meta.Path, meta.SizeBytes, meta.Checksum)
	}
	if meta.QcomflashPath != "9075.qcomflash.tar.gz" {
		t.Errorf("QcomflashPath = %q", meta.QcomflashPath)
	}

	// The 8275 must still publish both, for CLIs that resolve it from the generic fields.
	meta = VersionMetadata{Path: "stale.img", SizeBytes: 999, Checksum: "stale"}
	applyEDLFlashBundle(&meta, "dragonwing-iq-8275", "8275.qcomflash.tar.gz", 7, "sha8275")
	if meta.Path != "8275.qcomflash.tar.gz" || meta.SizeBytes != 7 || meta.Checksum != "sha8275" {
		t.Errorf("8275 lost its generic image route: path=%q size=%d sum=%q", meta.Path, meta.SizeBytes, meta.Checksum)
	}
}

// Promotion copies the generic image when there is one; an EDL version has none
// and must still be promotable on the strength of its bundle.
func TestHasPromotableImage(t *testing.T) {
	for name, tc := range map[string]struct {
		meta VersionMetadata
		want bool
	}{
		"generic image":       {VersionMetadata{Path: "x.img"}, true},
		"recovery flashpack":  {VersionMetadata{InstallMode: "recovery"}, true},
		"edl bundle, no path": {VersionMetadata{InstallMode: "edl", QcomflashPath: "x.tar.gz"}, true},
		"nothing at all":      {VersionMetadata{}, false},
		"edl but no bundle":   {VersionMetadata{InstallMode: "edl"}, false},
	} {
		t.Run(name, func(t *testing.T) {
			if got := hasPromotableImage(tc.meta); got != tc.want {
				t.Errorf("hasPromotableImage = %v, want %v", got, tc.want)
			}
		})
	}
}
