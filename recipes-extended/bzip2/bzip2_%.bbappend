...
# do_install_ptest.
#
# The tokens below must match the recipe's strings exactly, so they carry the
# ;destsuffix= form and the ${UNPACKDIR} paths oe-core uses. A :remove token
# that doesn't match anything is a harmless no-op.
