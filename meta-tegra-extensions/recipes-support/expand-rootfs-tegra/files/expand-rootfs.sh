...
PART_BASENAME="$(basename "$ROOTDEV")"   # e.g. nvme0n1p1
PARTNUM="$(cat "/sys/class/block/${PART_BASENAME}/partition")"
PARENT_SYS="$(readlink -f "/sys/class/block/${PART_BASENAME}/..")"
DISK="/dev/$(basename "$PARENT_SYS")"    # e.g. /dev/nvme0n1
echo "[expand-rootfs] RootDEV=$ROOTDEV Disk=$DISK Part=$PARTNUM FS=$FSTYPE"

# Skip if an A/B rootfs layout is detected (e.g. Mender). Tegra redundant
# layouts label the slots APP / APP_b; generic Mender layouts use rootfsa/b.
if lsblk -rno PARTLABEL "$DISK" 2>/dev/null | grep -qiE 'root[a-b]|app_b'; then
  echo "[expand-rootfs] Detected A/B rootfs layout -> skip."
  mkdir -p "$(dirname "$STAMP")"; touch "$STAMP"; exit 0
fi

# Fix GPT backup header when image is flashed to a larger disk.
