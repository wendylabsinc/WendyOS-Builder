...
func TestAddInstallsRouteInNetns(t *testing.T) {
	...
	// in the gateway's subnet - RouteAdd's kernel-side gateway resolution
	// depends on that connected route existing. testutils.NewNS() hands back
	// a bare namespace with only a down loopback, so give it the same
	// connected route a real container netns would already have before this
	// plugin runs.
	...
	if err := Add(targetNS.Path(), net.ParseIP("10.88.0.7"), args); err != nil {
		t.Fatalf("Add: %v", err)
	}
