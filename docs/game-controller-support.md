...
kernel must expose: one list of symbols that must resolve to `=y`, and one that may
resolve to `=m` provided the `kernel-module-*` package named beside it ships in the image.
