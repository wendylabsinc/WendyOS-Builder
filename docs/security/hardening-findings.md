...
| C3 | **Agent auto-updater installs the root control-plane binary with no signature/checksum check**, enabled by default (timer OnBoot+daily). NOTE: matches `origin/main` (updater present + referenced); the C3 fix is a **separate change** not carried by this branch. An earlier accidental partial deletion of the updater files on this branch broke the build and was reverted. | `recipes-core/wendyos-agent/files/download-wendyos-agent.sh`, `.../wendyos-agent-updater.{sh,timer,service}`, `.../wendyos-agent_1.0.bb` | test | 🔴 |
...
| H2 | **Default `wendy` user with `NOPASSWD: ALL`** — fleet-wide sudo escalation; remote root the moment `WENDYOS_SSHD=1`. **Fixed:** baked shared password removed (account locked, no login path) and the `NOPASSWD:ALL` grant deleted (postinst now scrubs any stale `/etc/sudoers.d/wendy` on OTA upgrade); `sudo` dropped from RDEPENDS. Nothing on-device escalated through this user (agent + all units run as root). | `recipes-extended/wendyos-user/wendyos-user_1.0.bb` | test | 🟢 |
...
| H4 | **mDNS advertises control-plane port + device UUID on ALL interfaces** despite a comment claiming `usb0`-only. **Fixed:** default `WENDYOS_MDNS_INTERFACES` set to the physical mgmt interfaces (`usb0,eth0,wlan0`) — discoverable over USB + LAN but not on the container bridge (cni0)/veths. Stays configurable (`?=`): set `""` for all, `"usb0"` for USB-only, or add a board's predictable NIC name. | `conf/distro/wendyos.conf` (`WENDYOS_MDNS_INTERFACES`); `recipes-connectivity/avahi/avahi_%.bbappend` | test | 🟢 |
| H5 | **Boot order allows USB/HTTP/PXE** — physical/LAN boot of attacker code (compounds C2). **Fixed:** both Tegra boot-priority overlays now list local storage only (`sd, nvme` / `nvme, sd`); USB/HTTP/PXE removed. Recovery is via NVIDIA recovery mode, unaffected. | `meta-tegra-extensions/recipes-bsp/tegra-bootcontrol-overlay/files/boot-priority*.dtso` | test | 🟢 |
| H6 | **No rootfs integrity (no dm-verity), rootfs mounted `rw`; `/data`,`/config`,`/boot` mounted without `nosuid,nodev,noexec`.** **Phase A done:** mount-flag hardening across the RPi fstabs + Jetson `data.mount` (`/data`=`nosuid,nodev`; `/config`,`/boot`=`nosuid,nodev,noexec`) — mount-flag guardrail green. **Phase B (dm-verity read-only rootfs) still open** — see `docs/security/specs/H6-dm-verity-readonly-rootfs-design.md`. | `recipes-core/base-files/files/*fstab`, `recipes-core/wendyos-data-setup/files/data.mount` | test (mount flags) | 🟢 mount flags / 🔴 verity |
...
| H8 | **Dev container runs `--privileged --network host`** with passwordless sudo + baked `dev:wendyos` password. **Partial:** baked `dev:wendyos` password removed. **Remaining (needs a Yocto-in-Docker build to validate):** dropping `--privileged`/`--network host` (may break `wic` image creation / host-cache access) and the NOPASSWD sudoers — build-gated like M3. | `scripts/docker/dockerfile`, `scripts/docker/docker-util.sh`, `Makefile` | test | 🟢 baked pw / 🔴 privileged+sudo |
