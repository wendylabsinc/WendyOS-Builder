...
| H2 | **Default `wendy`/`wendy` user with `NOPASSWD: ALL`** — fleet-wide static credential. **FIXED**: account created **locked** (no baked password) and the passwordless-sudo drop-in removed. Agent runs as root; dev console access is via root under `WENDYOS_DEV_LOGIN`, so nothing depends on wendy's sudo. Any dev-only wendy sudo must be added gated in `local.conf`, never fleet-wide. | `recipes-extended/wendyos-user/wendyos-user_1.0.bb` | test | 🟢 |
...
| H4 | **mDNS advertises control-plane port + device UUID on ALL interfaces** (comment wrongly claimed `usb0`-only). **RESOLVED (by decision)**: LAN discovery is kept by default so the CLI can find devices over WiFi/Ethernet — access is gated by the agent's mTLS (H3), so the advert grants nothing. Misleading comment fixed; `WENDYOS_MDNS_INTERFACES = "usb0"` is documented as an opt-in **fortress lockdown flag**. Guardrail now protects that lockdown mechanism from regressing. **Depends on H3** (agent must enforce mTLS). | `conf/distro/wendyos.conf`; `recipes-connectivity/avahi/avahi_%.bbappend` | test (lock-in) | 🟢 |
